...
_defs = dict(
    ...
    pred_days=15, threshold=0.50, quality_min=0.35,
)
...
# Constants defined here so FEATURE_COLS can reference them
N_SECTORS = 16
N_CAPS    = 3
...
FEATURE_COLS = [
    ...
    # Market regime (Nifty 50 above MA200 — macro bull/bear context)
    # This is a single scalar — no training/inference mismatch
    'mkt_regime',
    # Sector-demeaned features — stock vs sector peers
    # During training: raw_value − sector_mean (cross-sectional signal)
    # During single-ticker inference: raw_value (still informative as absolute level)
    'rsi_14_vs_sector',
    'price_vs_ma200_vs_sector',
    'adx_vs_sector',
    'atr_pct_vs_sector',
    'macd_hist_pct_vs_sector',
    # NOTE: sector_* and cap_* one-hot columns REMOVED — cause training/inference
...
def train_binary_ensemble(X_train, y_train):
    ...
    # XGBoost — balanced class weights, optimise PR-AUC
    m_xgb = XGBClassifier(
        n_estimators=300, max_depth=4, learning_rate=0.05,
        scale_pos_weight=spw,
        subsample=0.75, colsample_bytree=0.70,
        min_child_weight=12, gamma=0.25, reg_alpha=0.2, reg_lambda=2.5,
    ...
    m_xgb2 = XGBClassifier(
        ...
        min_child_weight=15, gamma=0.35, reg_alpha=0.3, reg_lambda=2.0,
        random_state=7, eval_metric='aucpr', n_jobs=-1,
        tree_method='hist',
    )
...
def predict_single(ticker, models_gc, models_dc,
    ...
    try:
        ...
        df_feat['mkt_regime'] = 0
    # Use raw feature as proxy for demeaned features (better than 0.0)
    _DEMEAN_PROXY = {
        'rsi_14_vs_sector': 'rsi_14',
        'price_vs_ma200_vs_sector': 'price_vs_ma200',
        'adx_vs_sector': 'adx',
        'atr_pct_vs_sector': 'atr_pct',
        'macd_hist_pct_vs_sector': 'macd_hist_pct',
    }
    for _vs, _raw in _DEMEAN_PROXY.items():
        if _vs not in df_feat.columns:
            df_feat[_vs] = df_feat[_raw].values if _raw in df_feat.columns else 0.0
...
# N_SECTORS and N_CAPS defined earlier before FEATURE_COLS
...
with st.sidebar:
    ...
    pred_days = st.slider("Prediction Window (days)", 10, 60, 30)
    ...
    st.markdown('<div style="font-size:12px;color:#9aa5c0;margin-bottom:8px;">💡 Higher threshold = fewer signals but better precision. Lower = more recall.</div>', unsafe_allow_html=True)
    threshold   = st.slider("Precision Threshold (Model A)", 0.25, 0.85, 0.50, 0.05,
                             help="Raise to improve precision (fewer false alarms). Lower to catch more GC events.")
    ...
    quality_min = st.slider("Minimum Quality Score", 0.0, 0.60, 0.35, 0.05,
    ...
    MODEL_PATH = "gc_model_v11.joblib"
    if os.path.exists(MODEL_PATH):
        ...
        if st.button("⚡ Load Saved Model", use_container_width=True,
                     help="Load previously trained model — no retraining needed."):
            try:
        ...
        st.markdown('<div style="font-size:12px;color:#5a6a8a;margin-bottom:6px;">No saved model — train first, then it saves automatically.</div>', unsafe_allow_html=True)
...
<div style="display:flex;align-items:center;gap:14px;margin-bottom:6px;">
  ...
  <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:#3d4d6a;
    ...
    v14 · Precision@K · Fixed Horizon · Calibrated Models · Better Generalization
  </div>
</div>
...
if train_btn:
    ...
    try:
        ...
        # ── Use ATR label directly — no blend (blend reduces positives) ──────────
        if 'target_gc_atr' in df_combined.columns:
            ...
            df_combined['target_gc'] = df_combined['target_gc_atr'].astype(int)
            log(f"  ✓ ATR target (direct): {int(df_combined['target_gc'].sum())} pos")
        ...
        try:
            payload = dict(
                ...
                pred_days=pred_days, models_trained=True,
                train_metrics=st.session_state.train_metrics,
            )
            joblib.dump(payload, MODEL_PATH, compress=3)
            size_mb = os.path.getsize(MODEL_PATH) / 1e6
            log(f"💾 Saved → {MODEL_PATH} ({size_mb:.1f} MB)")
            st.success(f"✅ Model saved to **{MODEL_PATH}** ({size_mb:.1f} MB) — click 'Load Saved Model' next time.")
    ...
    if bt_btn and bt_ticker.strip():
        ...
        with st.spinner(f"Backtesting {tk}..."):
            ...
            if df_raw is None or len(df_raw) < 400:
                ...
                train_horizon = st.session_state.get('pred_days', 30)
                horizon = bt_horizon  # display window for signal log

                df_feat_bt = engineer_features(df_raw,
                                fast_p=st.session_state.fast_p,
                                slow_p=st.session_state.slow_p)
                # Evaluate at training horizon — this is what the model was calibrated for
                df_lab_bt  = create_targets(df_feat_bt, n_days=train_horizon)

                WARMUP = 250
                df_eval = df_lab_bt.iloc[WARMUP:].copy().reset_index(drop=False)
                date_col = df_eval.columns[0]
                # Inject sector/cap one-hots and raw-feature proxies for _vs_sector
                _sid = SECTOR_MAP.get(tk, 15); _cid = CAP_MAP.get(tk, 1)
                for _i in range(N_SECTORS): df_eval[f'sector_{_i}'] = np.int8(1 if _sid==_i else 0)
                for _i in range(N_CAPS):    df_eval[f'cap_{_i}']    = np.int8(1 if _cid==_i else 0)
                if 'mkt_regime' not in df_eval.columns: df_eval['mkt_regime'] = 0
                _DPROXY = {'rsi_14_vs_sector':'rsi_14','price_vs_ma200_vs_sector':'price_vs_ma200',
                           'adx_vs_sector':'adx','atr_pct_vs_sector':'atr_pct',
                           'macd_hist_pct_vs_sector':'macd_hist_pct'}
                for _vs,_raw in _DPROXY.items():
                    if _vs not in df_eval.columns:
                        df_eval[_vs] = df_eval[_raw].values if _raw in df_eval.columns else 0.0
                X_eval = df_eval[FEATURE_COLS].replace([np.inf,-np.inf], np.nan).fillna(0)

                # ── Model predictions on every bar ─────────────────────────────
                # Use RAW ensemble (not calibrated) — Platt calibrator squashes
                # probabilities to ~0.5 on single-ticker data (trained on multi-ticker)
                ...
                # Quality-gated: fire only when prob AND quality both pass
                if st.session_state.model_quality_gc is not None and quality_min > 0:
                    try:
                        q_scores = np.clip(
                            st.session_state.model_quality_gc.predict(X_eval), 0, 1)
                        y_pred_quality = ((y_pred_a == 1) & (q_scores >= quality_min)).astype(int)
                    except Exception:
                        y_pred_quality = y_pred_a
                else:
                    y_pred_quality = y_pred_a

                # Primary metric: raw Model A (preserves recall)
                y_pred = y_pred_a
                ...
                if n_pos < 5:
                    st.warning(
                        ...
                        f"at {train_horizon}d training window. Try a longer history "
                        f"or increase Prediction Window in the sidebar."
                    )
                ...
                <div style="margin:18px 0 12px;">
                  ...
                  <span style="font-family:'JetBrains Mono';font-size:11px;color:#7a8aaa;margin-left:10px;">
                    {n_bars:,} bars · {n_years}y history · {n_pos} GC events · window={train_horizon}d (training) · display={horizon}d · threshold={threshold:.0%}
                  </span>
                </div>
                <div class="warn-box" style="margin-bottom:14px;">
                  TP = Model A signal AND GC confirmed within {train_horizon} days &nbsp;|&nbsp;
                  FP = Model A signal, NO GC in {train_horizon} days &nbsp;|&nbsp;
                ...
                # ── KPI row ─────────────────────────────────────────────────────
                # Compute Average Precision (PR-AUC) — correct metric for rare events
                # Accuracy is misleading: predicting "no GC" always gives ~94% accuracy
                from sklearn.metrics import average_precision_score as _aps
                try:
                    avg_prec_score = float(_aps(y_true, probs_gc))
                except Exception:
                    avg_prec_score = float('nan')
                ...
                with k4:
                    # Replace misleading Accuracy with PR-AUC
                    ap_c = "#00e5a0" if not np.isnan(avg_prec_score) and avg_prec_score > base_rate*2 else "#f5a623"
                    ap_str = f"{avg_prec_score:.3f}" if not np.isnan(avg_prec_score) else "n/a"
                    st.markdown(bkpi("Avg Precision (PR-AUC)",
                        ap_str,
                        f"baseline={base_rate:.1%} · >2× baseline = good", ap_c),
                        unsafe_allow_html=True)
                with k5:
                ...
                st.markdown(
                    f'<div class="warn-box">⚠️ <b>Accuracy is not shown</b> — with only {base_rate:.1%} GC events, '
                    f'a model that never fires gets {1-base_rate:.1%} accuracy. '
                    f'Use <b>Precision</b> (signal quality), <b>Recall</b> (coverage), '
                    f'<b>PR-AUC</b> (overall ranking quality), and <b>Precision Lift</b> instead.</div>',
                    unsafe_allow_html=True)

                # ── Precision@K: rank by probability, evaluate top-K signals ──────
                st.markdown('<div class="sec-hdr">Precision@K — How accurate are the TOP signals?</div>',
                            unsafe_allow_html=True)
                sorted_idx = np.argsort(probs_gc)[::-1]
                pk_rows = []
                for k_pct in [1, 2, 5, 10, 15, 20]:
                    k = max(1, int(len(probs_gc) * k_pct / 100))
                    top_k_idx = sorted_idx[:k]
                    prec_k = float(y_true[top_k_idx].mean())
                    avg_prob = float(probs_gc[top_k_idx].mean())
                    pk_rows.append({
                        "Top K%": f"Top {k_pct}% ({k} bars)",
                        "Avg Prob": f"{avg_prob:.1%}",
                        "Precision@K": f"{prec_k:.1%}",
                        "vs Base Rate": f"{prec_k/max(base_rate,1e-9):.2f}×",
                        "TP in top-K": int(y_true[top_k_idx].sum()),
                    })
                pk_df = pd.DataFrame(pk_rows)
                st.dataframe(pk_df, hide_index=True, use_container_width=True)
                st.markdown(
                    '<div class="warn-box" style="border-color:rgba(77,166,255,.4);color:#4da6ff;">'
                    '💡 <b>Precision@K</b>: If you only act on the top-K most confident signals, '
                    'how often is the model right? Top 1-5% signals should show 3-10x lift over random. '
                    'This is more actionable than overall precision which includes weak signals.</div>',
                    unsafe_allow_html=True)

                # ── Precision vs Recall tradeoff table ─────────────────────────
                st.markdown('<div class="sec-hdr">Precision vs Recall — Pick your operating point</div>',
                            unsafe_allow_html=True)
                ...
                # Raw Model A at current threshold
                tp_a,fp_a,fn_a,pr_a,rc_a = _cm2(y_pred_a, y_true)
                # Quality-gated
                tp_q,fp_q,fn_q,pr_q,rc_q = _cm2(y_pred_quality, y_true)
                # Model B (high recall)
                tp_b,fp_b,fn_b,pr_b,rc_b = _cm2(y_pred_b, y_true)
                # AND logic
                tp_and,fp_and,fn_and,pr_and,rc_and = _cm2(y_pred_and, y_true)

                tradeoff_df = pd.DataFrame({
                    "Strategy": [
                        f"Model A (thr={threshold:.0%})",
                        f"Model A + Quality≥{quality_min:.2f}",
                        f"Model B (thr={bt_recall_thr:.0%})",
                        f"A AND B",
                    ],
                    "TP": [tp_a, tp_q, tp_b, tp_and],
                    "FP": [fp_a, fp_q, fp_b, fp_and],
                    "FN missed": [fn_a, fn_q, fn_b, fn_and],
                    "Precision": [f"{pr_a:.1%}", f"{pr_q:.1%}", f"{pr_b:.1%}", f"{pr_and:.1%}"],
                    "Recall":    [f"{rc_a:.1%}", f"{rc_q:.1%}", f"{rc_b:.1%}", f"{rc_and:.1%}"],
                    "F1": [f"{2*pr_a*rc_a/(pr_a+rc_a+1e-9):.3f}",
                           f"{2*pr_q*rc_q/(pr_q+rc_q+1e-9):.3f}",
                           f"{2*pr_b*rc_b/(pr_b+rc_b+1e-9):.3f}",
                           f"{2*pr_and*rc_and/(pr_and+rc_and+1e-9):.3f}"],
                })
                ...
                st.markdown(
                    ...
                    f'💡 <b>Quality gate</b> improves precision by {pr_q-pr_a:+.1%} '
                    f'(removes {fp_a-fp_q} false alarms) at cost of {fn_q-fn_a} extra misses. '
                    f'<b>A AND B</b> is the recommended entry signal for live trading.</div>',
                    unsafe_allow_html=True)
