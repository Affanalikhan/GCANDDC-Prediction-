"""
Golden Cross Predictor — Quant Edition v12
All v11 fixes retained. New in v12:

1. near_cross_score: continuous 0–1 convergence feature (gap proximity × closing velocity).
   Replaces the old binary near_cross flag. Used as a hard gate — no signal when score < 0.30
   (gap is wide AND moving away from cross). Suppresses false positives when crossover is far.

2. Trend filter (ADX > 20): explicit check, shown individually in UI chips and screener table.
   Previously mixed with Hurst in one regime_ok flag — now separated for clarity.

3. Regime filter (Price > MA200): new explicit check. Signals only fire when stock is in a
   macro bull regime (above 200-bar MA). Shown individually in UI chips and screener table.

4. Composite signal gate: model_prob > 0.6 AND near_cross_ok AND trend_ok (ADX > 20).
   This is the primary entry condition in both predict_single() and backtest_strategy().
   Price > MA200 (regime_price_ok) and Hurst > 0.45 are additional filters on top.

5. Screener updated: filter status shown per ticker (ADX · MA200 · Near), composite-gate
   stocks marked with ⭐, ranking weighted by filter pass rate.
"""
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f'All NSE sectors represented</span></div>',
        unsafe_allow_html=True)
...
<div style="display:flex;align-items:center;gap:14px;margin-bottom:6px;">
  ...
  <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:#3d4d6a;
    ...
    v12 · Near-Cross Gate · ADX Trend Filter · Price>MA200 Regime · Composite Signal Gate
  </div>
</div>
...
if train_btn:
    ...
    try:
        ...
        # After concat the DatetimeIndex has duplicates (same date, different tickers).
        # We promote it to a column, sort by it, then reset to integer index.
        # walk_forward_split() will cut by position which now = chronological order.
...
with tab_model:
    ...
    st.markdown('<div class="sec-hdr">Critical Fixes — v11 (new) + v10 (inherited)</div>', unsafe_allow_html=True)
    fixes = [
...
# ═══════════════════════════════════════════════════════════════════
with tab_about:
    st.markdown("### About — Golden Cross Predictor v12")
